...
| **OFFLOAD_SAVE_POLICY** | enum | `round_robin` | Chunked LMCache save admission policy, including `lmcache_mp` PAGE-only/native-state and DSV4. `round_robin` preserves the compatibility behavior. `priority` admits reusable prefixes by rank-local demand and dirty-token cost; a finished request that cannot reserve capacity immediately is dropped instead of pinning KV indefinitely. |
| **OFFLOAD_SAVE_MIN_OBSERVED_COUNT** | int | `2` | Priority mode's hard reuse-demand floor. Aging and finished-request release benefit do not bypass it. |
