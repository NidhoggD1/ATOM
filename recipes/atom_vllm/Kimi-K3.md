...
persist those bytes under a valid prefix hash. The only safe source is
`SchedulerOutput.kv_connector_block_state.boundary_state_offloads`, which names
the exact block holding a committed boundary state. That structure exists in
**vLLM 0.29 and not in 0.28**.
...
- LMCache KV offload is supported on vLLM 0.29 with prefix caching and
  `--mamba-cache-mode align`; see above. Not available on 0.28 — the
  boundary-state hand-off the recurrent leg needs does not exist there.
