...
class DenseOffloadScheduler(OffloadSchedulerMixin, KVConnectorSchedulerBase):
    ...
    def get_num_new_matched_tokens(self, seq) -> tuple[int, bool]:
        ...
        if need <= 0:
            # `_clear_pending_load` keeps `_hit_save_floors`, so the floor set
            # just above survives and the request still saves only its tail.
            self._clear_pending_load(sid)
            return 0, False
    ...
    def _clear_pending_load(self, sid: str) -> None:
        ...
        # radix workload: 56 of 254 warm requests re-saved their entire prompt,
        # 22% of all bytes written D2H. The floor is dropped by
