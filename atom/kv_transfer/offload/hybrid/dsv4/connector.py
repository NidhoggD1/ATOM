...
class DSV4OffloadScheduler(OffloadSchedulerMixin, KVConnectorSchedulerBase):
    ...
    def build_connector_meta(self) -> LMCacheOffloadMetadata:
        ...
        # Saves: a finished priority candidate may already hold a reservation
        # made by request_finished(), because that callback runs after this
        # step's metadata was built. Dispatch those reservations first.
        if self._do_save:
            now = time.monotonic()
            committed = []
            ...
            for candidate in committed:
                ...
                if sid in self._save_inflight or sid in self._sidecar_save_inflight:
                    continue
                # Replacing the reservation with an operation leaves total
                # capacity unchanged.
                self._save_committed.pop(sid, None)
    ...
    def get_statistics(self) -> dict[str, int | float]:
        ...
        pinned = {
            sid: seq
            for sid, seq in self._finished_save_requests.items()
            if self._save_committed.get(sid) is seq
            or sid in self._save_inflight
            or sid in self._sidecar_save_inflight
        }
        candidate_scores = [
        ...
        statistics.update(
            ...
            save_pinned_blocks=sum(
                len(getattr(seq, "block_table", ())) for seq in pinned.values()
            ),
            save_pinned_tokens=sum(
                len(getattr(seq, "block_table", ())) * int(self.block_size)
                for seq in pinned.values()
            ),
            deferred_free_requests=len(pinned),
        )
