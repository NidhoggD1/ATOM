    ...
    def __post_init__(self):
        ...
        if self.speculative_config is not None:
            ...
            if not is_dspark:
                ...
                #
                # NOTE for indexer-CP A/Bs: the bound is a QUALITY policy, not a
                # structural limit -- nothing downstream carries a fixed depth,
                # and the CP chain was verified out of tree to be exactly equal
                # to the native kernel at query_len 8. A draft
                # checkpoint that declares no num_mtp_modules therefore caps
                # --num-speculative-tokens at 4 even though 7 runs correctly;
                # add the field to the draft config to raise it.
