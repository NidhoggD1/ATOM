...
class AtomOffloadWorkerMetadata(KVConnectorWorkerMetadata):
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"load_failed={self.load_failed}, state_stored={self.state_stored}, "
...
class AtomLMCacheOffloadConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def _collect_kda_stores(self, scheduler_output, preempted) -> list:
        ...
        ``scheduler_output.kv_connector_block_state`` is nulled again as soon as
        this method returns (vLLM does not ship it to the workers), so there is
        no later step in which to read it. A hand-off that is not consumed here
        is a boundary that is never stored, which the lookup cap then turns into
        a shorter hit -- quiet, and only visible as a hit rate that will not
        climb.
        """
        if self._kda_planner is None:
            return []
        state = getattr(scheduler_output, "kv_connector_block_state", None)
        offloads = getattr(state, "boundary_state_offloads", None) if state else None
        if not offloads:
            return []
        skip = set(preempted)
        skip.update(
