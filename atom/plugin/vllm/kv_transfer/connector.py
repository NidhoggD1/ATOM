...
class AtomLMCacheOffloadConnector(KVConnectorBase_V1):
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]) -> None:
        ...
        # Fold the layout vLLM resolved into the offload namespace, before the
        # engine (and with it the namespace) is built below. A block's bytes
        # are ordered differently under the two layouts M3's backends accept,
        # so a server that resolved the other one must miss rather than match
        # on a shared disk/remote backend. Worker-side only: the namespace is
        # only ever computed here.
        self._config.page_layout_tag = kv_layout_tag(tensors)
        ...
        logger.info(
            "ATOM LMCache offload: registered %d layers, num_blocks=%d, layout=%s",
            len(tensors),
            num_blocks,
            self._config.page_layout_tag,
        )
