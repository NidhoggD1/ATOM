...
class OffloadConfigShim:
    ...
    def __init__(self, vllm_config: Any) -> None:
        ...
        # Read by ``lmcache_replica_world_size``, on the scheduler side, where
        # there is no torch distributed group to count. Without it the
        # scheduler sizes a TP=4 replica as world=1 while the workers derive 4
        # from the aiter TP group.
        ...
        # Set by the worker once it has seen the KV tensors, and folded into
        # the offload namespace there. vLLM picks between two acceptable KV
        # layouts at startup and they pack a block's bytes differently, so the
        # resolved one has to be part of the key. Unknown until registration,
        # hence a slot rather than a value.
        self.page_layout_tag: str | None = None
