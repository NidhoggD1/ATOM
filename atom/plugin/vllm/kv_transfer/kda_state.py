...
* The KDA group in ``--mamba-cache-mode align`` cannot be read positionally at
  ...
  ``SchedulerOutput.kv_connector_block_state.boundary_state_offloads``, which
  names the exact block holding a committed boundary state.
...
class KdaBoundaryPlanner:
    ...
    def collect_stores(
        ...
        Consumed in the step it arrives, because that is the only step in which
        it exists: ``SchedulerOutput.kv_connector_block_state`` is nulled again
        as soon as ``build_connector_meta`` returns.
