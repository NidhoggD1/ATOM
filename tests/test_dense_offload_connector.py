...
def _drain_worker(worker):
    """Join a worker's copy pools.

    Upstream ATOM has `DenseOffloadConnector.close()` for this; the M3-AMD
    snapshot predates it, and adding it is out of scope for this port, so the
    tests join the executors directly the way the rest of this file does.
    Idempotent -- `ThreadPoolExecutor.shutdown` tolerates a second call.
    """
    worker._save_executor.shutdown(wait=True)
    worker._load_executor.shutdown(wait=True)
