...
def _break_carriers(tree: ast.Module) -> list[ast.FunctionDef]:
    """Every function that ends a capture segment.

    Found by decorator rather than by name: which method the sparse body hangs
    off has changed before (``_sparse_attn_run`` -> ``_forward_with_output``)
    and the guard is about the break existing, not about its spelling.
    """
    return [
        node
        for node in ast.walk(tree)
        if isinstance(node, ast.FunctionDef)
        and BREAK_DECORATOR in _decorator_names(node)
    ]


def test_sparse_attention_is_an_eager_break():
    carriers = _break_carriers(_module())
    assert carriers, (
        f"nothing in the M3 attention module carries @{BREAK_DECORATOR} -- M3's "
        "57 sparse layers go back inside the captured graph, with this batch's "
        "block_table, seq_lens and topk indices frozen into every replay"
    )
...
def test_the_break_writes_into_a_caller_owned_buffer():
    ...
    for carrier in _break_carriers(tree):
        assert "output" in {a.arg for a in carrier.args.args}, (
            f"{carrier.name} carries the eager break but allocates its own "
            "output; the break must receive the buffer instead"
        )
